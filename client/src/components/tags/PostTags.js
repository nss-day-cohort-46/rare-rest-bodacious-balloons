...
export const PostTags = () => {
    ...
    useEffect(() => {
        const tagIds = postById.tags?.map(tag => tag.id)
        setSavePostTags(tagIds)
    }, [postById])
    ...
    const handleTagChange = (event) => {
        if (event.target.name.includes('tag')) {
            ...
            if (event.target.checked) {
                newTag.push(parseInt(event.target.value))
            }
    ...
    const saveTagPost = () => {
        const savePostId = parseInt(postId)
        addPostTags({ tags: savePostTags, postId: savePostId })
                ...
                <div>
                    {
                        tags.map(t => <div key={t.id}>
                            <input type="checkbox"
                                ...
                                checked={savePostTags && savePostTags.find(pTag => pTag === t.id)}
                                value={t.id}>
                            </input>
